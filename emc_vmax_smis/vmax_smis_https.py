...
class HTTPSConnection(six.moves.http_client.HTTPSConnection):
    def __init__(self, host, port=None, key_file=None, cert_file=None,
        ...
        if not pywbemAvailable:
            print('Module PyWBEM not installed. Install PyWBEM using the python-pywbem package.')
        if six.PY3:
    ...
    def host_matches_cert(host, x509):
        ...
        msg = "Host %(host)s does not match x509 certificate contents: CommonName %(commonName)s."  \
              % {'host': host, 'commonName': common_name}

        if san_list is not None:
            msg = "%(message)s, subjectAltName: %(sanList)s." % {'message': msg, 'sanList': san_list}
    ...
    def set_context(self):
        ...
        if self.cert_file:
            try:
                ...
                msg = "Unable to load cert from %(cert)s %(e)s." % {'cert': self.cert_file, 'e': e}
                raise pywbem.cim_http.AuthError(msg)
            if self.key_file is None:
                ...
                try:
                    ...
                    msg = "No key file specified and unable to load key from %(cert)s %(e)s." \
                          % {'cert': self.cert_file, 'e': e}
                    raise pywbem.cim_http.AuthError(msg)
        ...
        if self.key_file:
            try:
                ...
                msg = "Unable to load key from %(cert)s %(e)s." % {'cert': self.cert_file, 'e': e}
                raise pywbem.cim_http.AuthError(msg)
        ...
        if self.ca_certs:
            try:
                ...
                msg = "Unable to load CA from %(cert)s %(e)s." % {'cert': self.cert_file, 'e': e}
                raise pywbem.cim_http.AuthError(msg)
        else:
...
def wbem_request(url, data, creds, headers=None, debug=0, x509=None,
    ...
    while numTries < tryLimit:
        ...
        try:
            ...
            msg = "Bad Status line returned: %(arg)s." % {'arg': arg}
            raise pywbem.cim_http.Error(msg)
        except socket.sslerror as arg:
            msg = "SSL error: %(arg)s." % {'arg': arg}
            raise pywbem.cim_http.Error(msg)
        except socket.error as arg:
            msg = "Socket error: %(arg)s." % {'arg': arg}
            raise pywbem.cim_http.Error(msg)
